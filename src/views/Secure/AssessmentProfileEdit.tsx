...
const AssessmentProfileEdit = () => {
    ...
    const [assessmentProfile, setAssessmentProfile] =
        useState<AssessmentProfileDTO>({} as AssessmentProfileDTO)
    ...
    useEffect(() => {
        ...
        const fetchAssessmentProfile = async () => {
            try {
                ...
                if (response.status === 200) {
                    setAssessmentProfile(response.data)
                    reset(response.data)
                } else {
                    console.error(
                        'Failed to fetch assessment profile:',
                        response.statusText
                    )
                }
            } catch (error) {
    ...
    const onSubmit = async (data: AssessmentProfileDTO) => {
        try {
            data.createdUserId = assessmentProfile.createdUserId
            data.id = assessmentProfile.id
            data.tenantId = assessmentProfile.tenantId
            data.createdDate = assessmentProfile.createdDate
            data.createdUserId = assessmentProfile.createdUserId
            data.updatedDate = new Date()
            data.updatedUserId = user?.userId
            const response = await axios.put(
                `${import.meta.env.VITE_BackendURL}/assessmentprofiles/${id}`,
                data,
                {
                    headers: {
            ...
            if (response.status === 200) {
                ...
                console.error(
                    'Failed to update assessment profile:',
                    response.statusText
                )
            }
        } catch (error) {
...
export default AssessmentProfileEdit
